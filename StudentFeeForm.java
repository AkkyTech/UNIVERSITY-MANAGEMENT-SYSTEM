...
public class StudentFeeForm extends JFrame implements ActionListener, ItemListener {
    ...
    public void itemStateChanged(ItemEvent ie){
        try{
               ...
               ResultSet rss=conn.s.executeQuery("select s_name,father_name,course,branch from studentdetails where roll='"+str+"'");
               while(rss.next())
               {
                   t.setText(rss.getString("s_name"));
                   t1.setText(rss.getString("father_name"));
                   t2.setText(rss.getString("course"));
                   t3.setText(rss.getString("branch"));
               }
        }
    ...
    public void actionPerformed(ActionEvent ae)
    {
        if(ae.getSource()==b1)
        {
            ...
            try{
                    connection c=new connection();
                    c.s.executeUpdate("insert into fee values('"+s+"','"+s1+"','"+s2+"','"+s3+"','"+s4+"','"+s5+"','"+s6+"')");
